...
import com.example.tugas14.models.UserSessionModel;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public DBHelper(Context context) {
        super(context, "tugas14.db", null, 1);
    }
    ...
    public ResponseModel<List<UserLocationModel>> getLocations(int userId) {
        ResponseModel<List<UserLocationModel>> responseModel = new ResponseModel<>();
        try {
            SQLiteDatabase DB = this.getWritableDatabase();
            Cursor cursorSession = DB.rawQuery("SELECT * FROM user_locations WHERE user_id = ?", new String[]{String.valueOf(userId)});
            responseModel.Data = new ArrayList<>();
            while (cursorSession.moveToNext()) {
                double latitude = cursorSession.getDouble(2);
                double longitude = cursorSession.getDouble(3);
                Date createdAt =  new Date(cursorSession.getLong(4));
                responseModel.Data.add(new UserLocationModel(cursorSession.getInt(0),cursorSession.getInt(1), latitude, longitude, createdAt));
            }
            responseModel.Status = true;
            return responseModel;
        } catch (Exception e) {
